...
const Kubernetes: React.FC = () => {
    ...
    return (
      <div
        style={{
          ...
          overflowX: "hidden", // Évite un défilement horizontal inutile
  ...
  <section>
    ...
    <p>
      Les services Kubernetes répondent à plusieurs besoins :
    </p>
    <ul>
      <li>Offrir une adresse réseau stable et permanente, même lorsque les pods changent ou redémarrent.</li>
      <li>Distribuer le trafic entre plusieurs pods (équilibrage de charge).</li>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>Expose le service sur un port spécifique de chaque nœud du cluster.</td>
        ...
        <tr>
          ...
          <td>Mappe un nom DNS interne Kubernetes vers un nom DNS externe.</td>
...
spec:
  ...
  type: ClusterIP
    ...
    <ul>
      <li>
        <strong>selector</strong> : Permet de cibler les pods ayant des labels spécifiques 
        (ici, <code>app: mon-application</code>).
      </li>
      <li>
        <strong>ports</strong> : Décrit les ports exposés par le service :
        <ul>
          <li><code>port</code> : Port utilisé par les clients.</li>
          <li><code>targetPort</code> : Port interne du pod.</li>
        </ul>
      </li>
      <li>
...
spec:
  ...
  type: ClusterIP
    ...
    <p>
      Par défaut, Kubernetes distribue les requêtes de manière équilibrée (round-robin) entre les pods. 
      Si une application nécessite de maintenir une session utilisateur constante vers le même pod, 
      vous pouvez utiliser le champ <code>sessionAffinity</code>.
    </p>
...
spec:
  ...
  <section>
    ...
    <ul>
      <li><strong>Lister tous les services :</strong> <code>kubectl get services</code></li>
      <li><strong>Inspecter un service :</strong> <code>kubectl describe service &lt;nom-du-service&gt;</code></li>
      <li>
        <strong>Tester un service interne :</strong> 
        <code>curl http://&lt;nom-service&gt;.&lt;namespace&gt;.svc.cluster.local</code>
      </li>
    </ul>
  </section>
  ...
  <section>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          <td><strong>sessionAffinity</strong></td>
          <td>Maintenir une session utilisateur constante vers un pod.</td>
